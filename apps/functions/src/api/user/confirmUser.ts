...
export const confirmUser = https.onCall(async (uid: string, context) => {
    try {
        if (!context.auth?.token.admin) throw new Error('User does not have permission')

        // UPDATE AUTH USER
        const auth = getAuth()
        await auth.setCustomUserClaims(uid, { userConfirmed: true })
        ...
        userData.claims = { ...userData.claims, userConfirmed: true }
