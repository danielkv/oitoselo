...
Deno.serve(async (req: Request) => {
    try {
        ...
        if (!userId || !action) return new Response('Missing data', { status: 422, headers: corsHeaders })

        await client.rpc('set_claim', { uid: userId, claim: 'claims_admin', value: action === 'promote' })
